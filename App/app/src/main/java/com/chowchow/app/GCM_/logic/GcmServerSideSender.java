 ...
 */
package com.chowchow.app.GCM_.logic;

import static com.chowchow.app.GCM_.logic.HttpRequest.CONTENT_TYPE_FORM_ENCODED;
import static com.chowchow.app.GCM_.logic.HttpRequest.CONTENT_TYPE_JSON;
import static com.chowchow.app.GCM_.logic.HttpRequest.HEADER_CONTENT_TYPE;
import static com.chowchow.app.GCM_.logic.HttpRequest.HEADER_AUTHORIZATION;
...
public class GcmServerSideSender {
            ...
            case RESPONSE_PLAINTEXT_MESSAGE_ID:
                ...
                if (lines.length > 1) {
                    ...
                    if (secondLineValues.length == 2
                        ...
                        Log.v("gcm_info", "Message sent: canonical registration id = "
                                + secondLineValues[1]);
                    } else {
                        Log.v("gcm_error", "Invalid response from GCM."
                                + "\nResponse: " + httpRequest.getResponseBody());
                    }
                }
                break;
            case RESPONSE_PLAINTEXT_ERROR:
                Log.v("gcm_error", "Message failed.\nError: " + firstLineValues[1]);
                break;
            default:
                Log.v("gcm_error", "Invalid response from GCM."
                        + "\nResponse: " + httpRequest.getResponseBody());
    ...
    public void sendHttpJsonDownstreamMessage(String destination,
        ...
        try {
            ...
            Log.v("gcm_error", "Failed to build JSON body");
            return;
        }
        ...
        try {
            ...
            Log.v("gcm_info", "Send message:\n" + jsonResponse.toString(2));
        } catch (JSONException e) {
            Log.e("gcm_error", "Failed to parse server response:\n"
